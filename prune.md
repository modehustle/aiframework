...
>
> Run this on a **strong model** — re-deriving architecture from code is `/make_task`-grade reasoning, not work for a cheap executor.
...
4. **Contradictions are a fork, not your call.** If the code contradicts a recorded decision, you do not silently "fix the doc". Flag it and let the user choose:
   ...
   - *the doc is right, the code drifted* → this is a **bug**; the user schedules a `/make_task`. You do not touch code here.
...
5. Report: which files changed, how many `DECISIONS` entries were archived, and any **forks left for the user** (doc-vs-code contradictions that may be bugs needing `/make_task`).
