...
- `verb_section_filled` reads any `<…>` as an unfilled placeholder, so a section that is CREATED with a hint comment in it (the build journal's lessons) can never be "filled" by it — such a section needs its own check that strips comments first (`build_lessons_filled`)
- Cleaning up after a build deletes checkouts and branches: before any deletion prove the build branch is an ancestor of the trunk HEAD (`git merge-base --is-ancestor`) — deleting an unmerged build branch loses the whole build, and git does not ask
