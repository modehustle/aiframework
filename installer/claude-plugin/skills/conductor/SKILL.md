...
**Тяжесть подзадачи — отдельным полем.** Роль говорит, *какой процедурой* исполнять;
`**Tier**:` — *насколько тяжёлая именно эта подзадача*. Без него все воркеры сборки уедут
на одну модель: на практике каждая подзадача называет `run-task`, а её класс во frontmatter
— `cheap`. «Причесать строки ошибок» и «спроектировать модуль авторизации» — обе `run-task`
и совсем не одна работа.
...
Поле необязательное: без него берётся класс самой процедуры.

**Роль и tier, а не модель.** План переезжает между машинами, а `haiku` — факт про подписки
одного человека.

Таблица отвечает в пять шагов, и третий делает её почти бесплатной:

| | ключ | когда срабатывает |
|---|---|---|
| 1 | `role_<процедура>` | эта процедура настроена особо — намеренно и редко, поэтому бьёт всё |
| 2 | `tier_<из плана>` | дирижёр сказал, что подзадача тяжёлая |
| 3 | `tier_<процедуры>` | по классу, объявленному самой процедурой |
| 4 | `role_default` | один ответ на всё |
| 5 | встроенный | ничего не задано |
