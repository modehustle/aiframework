...
```
fraim conduct "сделай сервис коротких ссылок"
```

Поднимает агента-дирижёра в терминале текущего чекаута — на том агенте и той модели, что
назначены роли `conductor` (`fraim roles set conductor`), — и передаёт ему задачу. Без среды
исполнения печатает задание, чтобы запустить дирижёра вручную.
