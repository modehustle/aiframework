...
verb_plan_context_paths() {
    [ -f "$1" ] || return 0
    # One entry per line, and the path is the FIRST backticked token on it: an entry reads
    # `path` · `symbol` — why. Taking every token would read the anchor as a second path and
    # then fail the plan over a symbol that was never meant to be a file.
    awk '/^## Codebase Context/{f=1;next} /^## /{f=0} f' "$1" |
        sed -n 's/^[^`]*`\([^`]*\)`.*/\1/p' | grep -E '/|\.[a-z]+$' | sort -u
}
...
$_bad"
    ...
    dim "  дальше: /run-task — здесь или в новом чате"
}
