...
Берись за режим флота, когда выполнены **все три** условия:
...
Если хоть одно не выполнено — обычная реактивная работа, а где её надо передать другой
сессии, [/make-task](make-task.md) и [/run-task](run-task.md). Флот на связанных кусках не
ускоряет, а добавляет слияние к той же работе.
