...
Четыре числа, и все они выведены из одного дефицита — **приёмка одна на сборку**. Потолок
...
то есть приёмка, которой не существует. Работает то из ограничений, что жёстче.
...
- **один файл у двух подзадач одной волны.** Это несущая проверка режима, а не удобство: два
  воркера, один файл — git сольёт обе правки молча, без конфликта, и приёмка «одним
  решением» окажется ложью. Границу сборки задаёт именно эта проверка
...
в ствол собранное уводит человек после приёмки: гейт стоит **до** записи в ствол
...
не знает исключений для режима флота.

**Приёмка — ратификация, а не конец сборки.** Она не двигает ствол (гейт стоит до записи в
него) и ничего не убирает. Поэтому её исход печатает, что за сборкой ещё числится: ветка
сборки и в стволе она или нет, чекауты воркеров, задания, ненаписанные уроки, — и одну
команду, которая это закрывает. Дальше два шага, и оба обязательны.
...
Комментарий-подсказку внутри раздела можно не трогать: уборка считает раздел пустым по
тексту, а не по наличию комментария.

---

## Шаг 9. Убрать за сборкой

```
fraim dispatch close СБОРКА
```

Сначала уведи принятое в ствол — это твой шаг, командой его за тебя никто не делает
([MODES.md §4](../MODES.md#4-гейт-относительно-ствола-развилка-ab), вариант A). Потом эта
команда закрывает сборку: освобождает воркеров в среде (операция №6), снимает их чекауты,
удаляет ветки воркеров и ветку сборки, убирает задания из `ai/parallel/`, дописывает в
журнал раздел «## Уборка» и ставит маркер `closed`.

**Это гейт, а не сценарий.** Он отказывает и не трогает ничего:

- **сборка не принята** — убирать до ратификации значит удалить то, по чему принимают;
- **раздел «## Уроки дирижёра» пуст** — после уборки их будет не из чего вспомнить;
- **ветка сборки не в стволе** — удалить её сейчас значит потерять всю сборку. Единственный
  отказ, который защищает работу, а не порядок;
- **в чекауте воркера есть незакоммиченное** — снос чекаута уничтожит работу, которую никто
  не видел;
- **работа чекаута не в ветке сборки** — она не собрана, и доказать это может только ветка.

Убирается ровно то, что сборка создала ([B6](../PRINCIPLES.md#b6-глагол-трогает-только-то-что-сам-изменил)):
если в `ai/parallel/<сборка>/` лежит что-то кроме `task.md`, папка перестаёт быть нашей и
остаётся на месте.

**Забыть этот шаг нельзя молча.** Пока маркера нет, `fraim status` держит находку «сборка
принята, но за ней не убрано», а `fraim clean` предлагает уборку в своём плане — и печатает
отказ гейта именем и причиной, если убирать ещё рано. Первый живой прогон кончился ровно
здесь: сборка была принята, а три чекаута, три ветки воркеров, ветка сборки и пустые уроки
пережили её и нашлись руками.
...
| терминалы и воркчекауты после сборки | **среды** | `worker-release`, операция №6 |
| чекауты git, ветки воркеров и ветка сборки | **наше** | git, `fraim dispatch close` |
