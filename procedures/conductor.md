...
**Отправленная команда — не подтверждённая модель.** Дирижёр печатает текст в терминал
воркера и не может убедиться, что модель правда переключилась: читать вывод терминала как
доказательство значило бы поверить прозе той же стороны, которую `verify` (шаг 5) нарочно
проверяет по дереву, а не по отчёту, — здесь просто нет дерева, по которому сверить модель.
Вместо того чтобы изображать уверенность, которой нет, `fraim dispatch run` пишет в
`fleet.tsv` честный статус на каждую подзадачу — `launched` (модель ушла флагом при
старте), `session` (команда отправлена, не подтверждена) или `dropped` (агент не знает, как
переключиться, — работает на своей модели по умолчанию). `fraim dispatch watch` показывает
его рядом со статусом воркера, а `fraim dispatch accept` называет вслух перед приёмкой,
какие подзадачи ушли без подтверждения, — это не блокирует приёмку (решает человек, не
гейт), но не даёт находке пройти молча.
